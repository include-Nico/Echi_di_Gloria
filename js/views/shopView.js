...
export function renderShop() {
  ...
  // Carica il database dallo stato globale
  const FULL_CARD_DB = gameState.databases.cards;
  
  // Offerta giornaliera fissa (o prima carta disponibile come fallback)
  const dailyCard = FULL_CARD_DB.find(c => c.id === 'JAP_008') || FULL_CARD_DB[0]; 
  const dailyPrice = 450;
  const originalPrice = 650;

  // Definizione dei pacchetti disponibili
  const packs = [
    { 
      id: 'standard', name: 'Standard', desc: '1 Non Comune Garantita', cost: 50, currency: 'silver', 
      art: 'https://lh3.googleusercontent.com/aida-public/AB6AXuAQLz2KyMcLOfs1r6-uM3fTKqdlemkwTYMiMjJcH6qu7TlmIE52UXwqBjxibYoory-FDFAb-oDnl_9Rlx3KOK73idvj0mpJ604YUKedW0JWo3VWBd3FQfUdZIbgOlM1q8JIh3e-x2xyFZQ6_UlaYS9b6icQKm3d837-rYsVNTkHHJ15Q4ndlX0GRbllZ7wbbOzMPjG7jYtuD13ytO6eSjKebmPKjqwYq8loYOdBw0fbv21EAdRdsgPXjA', 
      faction: null, guarantee: 'Non Comune' 
    },
    { 
      id: 'vichinghi', name: 'Vichinghi', desc: '1 Rara Garantita', cost: 100, currency: 'silver', 
      art: 'https://lh3.googleusercontent.com/aida-public/AB6AXuCJ_TMBRWC-iWUf3Yc5x2w_3apKtYYt3gos1HFwHakmHtduUrx3PKM7yetQCiDMJF6RlnxCwLeFG38pN04QLz9dq_p1qQ_if1qgUStwGXs-pXr5223EP3fyVIicZZ0yCFRKn0F3jTgZYBJBjTk8jpJW8E-HE9R-0lcHeFh3XDiWKyibTjP5DxlrD2mNvzXKgWtlZH88sfROIT61q9YQL2mrb61L-RkrM_NQFSZP_EUZq7NnrbLzEepDqQ', 
      faction: 'Vichinghi', guarantee: 'Rara' 
    },
    { 
      id: 'medioevo', name: 'Medioevo', desc: '1 Rara Garantita', cost: 100, currency: 'silver', 
      art: 'https://lh3.googleusercontent.com/aida-public/AB6AXuDWjRSdUgKcd0Ed2DO8CySiDhg8np1fu_uAgROGnMHDKXmL3OzBqgdLkuXUBRpjaP0zjkvPV9EhFP4r9ByvuNNrllmtDRNYj4G_V7d3QJ5o0_zkfteuCwRTESmMQx3DOWG0nrMAVobrMWZOduw198K7bGdc_eaKo9kA8idGphgmGUru7Yty5zn27DWJy4qN3wMtHr0_ZNJ_XBHHWVsjVfi8m4dSr2fKiXckTnAFrkQ68g0gahD_8vF4iQ', 
      faction: 'Medioevo', guarantee: 'Rara' 
    },
    { 
      id: 'giapponesi', name: 'Giapponesi', desc: '1 Rara Garantita', cost: 100, currency: 'silver', 
      art: 'https://image.pollinations.ai/prompt/dark%20fantasy%20japanese%20shrine%20booster%20pack%20foil%20card%20art?width=400&height=300&nologo=true', 
      faction: 'Giapponesi', guarantee: 'Rara' 
    },
    { 
      id: 'nativi', name: 'Nativi', desc: '1 Rara Garantita', cost: 100, currency: 'silver', 
      art: 'https://image.pollinations.ai/prompt/dark%20fantasy%20native%20american%20totem%20booster%20pack%20foil%20card%20art?width=400&height=300&nologo=true', 
      faction: 'Nativi', guarantee: 'Rara' 
    },
    { 
      id: 'leggendario', name: 'Leggendario', desc: '1 Epica o Legg. Garantita', cost: 200, currency: 'gems', 
      art: 'https://lh3.googleusercontent.com/aida-public/AB6AXuD5RvjcrHvsSqu9HWfgGZneQJb_rw7lFu3kxbTPd-PJZOlVVGcZ9zYdz-0qIOuuQjnYmy1mXBsKZgkvU-9CHgnN4GkUcScVrK_WBq0Tp2J_VW0-PR1ZaLBBQ1v3qsQPxg6fcD8adMVtfroWar9xWcx2mkgungtym0rVA4vis7xfiqQSvLYeC_LryyM2Ec-TCUhNNtc8rCBY85JHA54CM0eARzXJwfYg-k3ZStmRKL4hUONIZlPJOq8sGQ', 
      faction: null, guarantee: 'Epica' 
    }
  ];
      ...
      <div class="p-4 flex flex-col gap-3">
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-2">
            <span class="inline-flex items-center justify-center px-1.5 py-0.5 bg-primary text-on-primary rounded font-tactical text-[9px] uppercase font-bold">
              Offerta 24h
            </span>
            <span class="text-on-surface-variant font-tactical text-[10px] flex items-center gap-1">
              <span class="material-symbols-outlined text-[14px]">schedule</span> 14:22:08
            </span>
          </div>
...
// ---------------------------------------------------------
// ALGORITMO GACHA E APERTURA
// ---------------------------------------------------------

function openBoosterPack(packDef, FULL_CARD_DB) {
  // 1. Definisci il pool filtrando per fazione (se richiesto dal pacchetto)
  let pool = FULL_CARD_DB;
  if (packDef.faction) {
    pool = FULL_CARD_DB.filter(c => c.faction === packDef.faction);
  }
  if(pool.length === 0) pool = FULL_CARD_DB; // Sicurezza anti-crash
  ...
  // 3. Funzione di ricerca per rarità
  const getCardByRarity = (targetRarity) => {
    ...
    // Se la fazione non ha quella rarità (es. Nativi Leggendaria mancante), estrai dal database globale
  ...
  // 5. Popola il pacchetto
  const pulledCards = [];
  for (let i = 0; i < 4; i++) {
    pulledCards.push(rollRandomCard());
  }
  
  // Aggiunge l'ultima carta garantita
  ...
  // 6. Passa le carte all'animazione
  renderPackRevealModal(pulledCards);
}

// ---------------------------------------------------------
// ANIMAZIONI 3D E RIVELAZIONE
// ---------------------------------------------------------
...
function renderPackRevealModal(cards) {
  ...
  // Stili in linea per rotazioni 3D e glowing condizionale
  const styles = `
    <style>
      ...
      @keyframes shake-legendary {
        ...
        25% { transform: translateX(-6px) rotate(-6deg) scale(1.05); box-shadow: 0 0 40px #f2ca50; }
        50% { transform: translateX(6px) rotate(6deg) scale(1.05); box-shadow: 0 0 60px #f2ca50; }
        75% { transform: translateX(-6px) rotate(-6deg) scale(1.05); box-shadow: 0 0 40px #f2ca50; }
      }
  ...
  const cardsHtml = cards.map((c, i) => {
    ...
    <div class="perspective w-full max-w-[120px] aspect-[5/7] mx-auto opacity-0 translate-y-10 transition-all duration-500 card-slot" data-index="${i}" data-rarity="${c.rarity}">
      <div class="preserve-3d w-full h-full relative rotate-y-180 card-inner cursor-pointer rounded overflow-hidden border border-outline-variant glow-${rarityClass}">
        ...
        <!-- FRONTE (Mostrato dopo l'animazione) -->
    ...
    cardSlots.forEach((slot, i) => {
      ...
      setTimeout(() => {
        if (rarity === 'Leggendaria') {
          slot.style.animation = 'shake-legendary 1.2s ease-in-out';
          setTimeout(() => { inner.classList.remove('rotate-y-180'); }, 1200);

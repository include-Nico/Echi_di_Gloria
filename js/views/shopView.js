...
export function renderShop() {
  ...
  // Dati negozio
  const dailyCard = FULL_CARD_DB.find(c => c.id === 'JAP_008'); 
  ...
  container.innerHTML = `
    ...
    <div class="relative overflow-hidden rounded-xl bg-surface-container-high shadow-xl border border-primary/20">
      ...
      <div class="p-4 flex flex-col gap-3">
        ...
        <div class="flex gap-4 items-center">
          <div class="relative w-24 aspect-[5/7] rounded overflow-hidden shadow-lg border border-primary">
    ...
    <div class="flex flex-col gap-2 mt-2">
      <div class="flex items-center justify-between">
        ...
        <span class="font-tactical text-[9px] text-on-surface-variant">Trascina per esplorare</span>
      </div>
    ...
    btn.addEventListener('click', () => {
      ...
      if (pack.currency === 'silver') {
        ...
        if (gameState.currencies.gems >= pack.cost) {
          ...
          updateCurrencyUI();
          openBoosterPack(pack);
        } else showToast("Gemme insufficiente.");
      }
    });
  });
...
// ALGORITMO GACHA
function openBoosterPack(packDef) {
  ...
  const getCardByRarity = (targetRarity) => {
    ...
    if (filtered.length === 0) filtered = FULL_CARD_DB.filter(c => c.rarity === targetRarity) || pool; 
  ...
  const pulledCards = [];
  for (let i = 0; i < 4; i++) pulledCards.push(rollRandomCard());
  
  if (packDef.guarantee === 'Epica') pulledCards.push(getCardByRarity('Epica/Leggendaria'));
  else pulledCards.push(getCardByRarity(packDef.guarantee));

  renderPackRevealModal(pulledCards);
}

// ANIMAZIONI 3D SVELAMENTO CARTE
function renderPackRevealModal(cards) {
  ...
  // Stili CSS dinamici per flip 3D e glowing
  const styles = `
    <style>
  ...
  const cardsHtml = cards.map((c, i) => {
    // Rimuove lo spazio da "Non Comune" per i selettori CSS
    ...
    <div class="perspective w-full max-w-[120px] aspect-[5/7] mx-auto opacity-0 translate-y-10 transition-all duration-500 card-slot" data-index="${i}" data-rarity="${c.rarity}">
      <div class="preserve-3d w-full h-full relative rotate-y-180 card-inner cursor-pointer rounded overflow-hidden">
        
        <!-- FRONTE CARTA (Inizialmente nascosto dalla rotazione) -->
        <div class="backface-hidden absolute inset-0 rounded border border-outline-variant glow-${rarityClass}">
          ...
          <div class="absolute bottom-0 inset-x-0 bg-surface-container-lowest/90 px-1 py-1 text-center font-tactical font-bold text-[8px]">
            <span class="${c.rarity === 'Leggendaria' ? 'text-primary' : (c.rarity === 'Epica' ? 'text-tertiary' : 'text-on-surface')}">${c.rarity.toUpperCase()}</span>
          </div>
        </div>
        
        <!-- RETRO CARTA (Visibile all'inizio) -->
        <div class="backface-hidden rotate-y-180 absolute inset-0 bg-surface-container-highest border-2 border-outline-variant flex items-center justify-center rounded bg-[url('https://www.transparenttextures.com/patterns/wood-pattern.png')]">
  ...
  modal.innerHTML = `
    ...
    <button id="closeRevealBtn" class="mt-12 px-6 py-2.5 bg-surface-container text-on-surface-variant font-tactical font-bold rounded-full border border-outline-variant hover:text-primary transition-colors opacity-0 pointer-events-none">CHIUDI E AGGIORNA GRIMORIO</button>
  `;
  ...
  // Fase 1: Apparizione delle carte (dorso coperto)
  ...
  // Fase 2: Svelamento automatico sequenziale (Flip 3D)
  setTimeout(() => {
    cardSlots.forEach((slot, i) => {
      const rarity = slot.dataset.rarity;
      const inner = slot.querySelector('.card-inner');
      
      let delay = i * 400; // Timing base
      
      setTimeout(() => {
        if (rarity === 'Leggendaria') {
          // Animazione preparatoria per le Leggendarie
          slot.style.animation = 'shake-legendary 1.2s ease-in-out';
          setTimeout(() => {
            inner.classList.remove('rotate-y-180'); // Esegue il flip
          }, 1200);
    ...
    // Mostra il pulsante di chiusura alla fine di tutte le animazioni
    setTimeout(() => {

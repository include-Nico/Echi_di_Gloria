...
export function renderProfileModal() {
    ...
    <div class="w-full max-w-sm bg-surface-container-low border border-outline-variant p-5 rounded-xl shadow-2xl flex flex-col gap-5">
      ...
      <div class="flex flex-col gap-2">
        ...
        <div class="flex gap-2">
          ...
          <button id="btnChangeName" class="bg-primary hover:bg-primary-fixed-dim text-on-primary font-tactical font-bold text-xs px-3 rounded shadow flex items-center gap-1 active:scale-95 transition-all">
            <span class="material-symbols-outlined text-[14px] text-secondary">diamond</span> ${currentCost}
          </button>
        </div>
      ...
      <!-- Zona Pericolo -->
      <div class="mt-4 pt-4 border-t border-error/30 flex flex-col gap-3">
        ...
        <button id="btnDeleteAccount" class="w-full py-3 bg-transparent border border-error text-error hover:bg-error/20 font-tactical font-bold text-xs rounded active:scale-95 transition-all flex justify-center items-center gap-2">
          <span class="material-symbols-outlined text-[16px]">delete_forever</span> ELIMINA ACCOUNT DEFINITIVAMENTE
        </button>
      </div>
    </div>
  `;
  ...
  modal.querySelector('#btnChangeName').onclick = () => {
    ...
    if (gameState.currencies.gems >= currentCost) {
      ...
      // Update Button Cost UI
      const nextCost = 10 * Math.pow(2, gameState.player.nameChangeCount);
      modal.querySelector('#btnChangeName').innerHTML = `<span class="material-symbols-outlined text-[14px] text-secondary">diamond</span> ${nextCost}`;
      
      input.value = '';
      err.classList.add('hidden');
      
      // Sincronizza UI Header / Modali
  ...
  modal.querySelector('#btnDeleteAccount').onclick = () => {
    const confirmDelete = confirm("L'oblio ti attende. Sei sicuro di voler eliminare la tua anima dal database?");
    if (confirmDelete) {
      localStorage.clear();
